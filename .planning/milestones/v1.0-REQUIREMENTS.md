# Requirements Archive: v1.0 Tuning Wizard
...
- [x] **COMP-01**: Before/after comparison view showing current config vs selected preset
- [x] **COMP-02**: Comparison organized by system grouping (combat, economy, progression, stats, etc.)
- [x] **COMP-03**: Derived metrics shown prominently (time-to-kill, XP-to-level, gold-per-hour at key levels)
- [x] **COMP-04**: Raw field diffs available in expandable sections below derived metrics
- [x] **COMP-05**: Color-coded changes: increases, decreases, unchanged — context-aware (e.g., lower XP-to-level = good)

### Apply Flow

- [x] **APPLY-01**: Per-section accept/reject checkboxes for each system category
- [x] **APPLY-02**: Config snapshot taken before any apply operation (undo point)
- [x] **APPLY-03**: Undo last apply restores config to pre-apply snapshot
- [x] **APPLY-04**: Reset button discards all wizard state and returns to current config
- [x] **APPLY-05**: Applied values written to configStore and saved to application.yaml
...
- [x] **UI-04**: Contextual tooltips on every tunable field explaining what it does and what it interacts with
- [x] **UI-05**: Search/filter across parameter names and descriptions
- [x] **UI-06**: Post-apply health check surfaces any problematic combinations from cherry-picked sections

### Visualization

- [x] **VIZ-01**: XP curve chart (Recharts) showing XP-per-level for current vs preset config
- [x] **VIZ-02**: Mob tier power chart showing HP, damage, armor, XP across tiers at selectable levels
- [x] **VIZ-03**: Stat profile radar chart comparing current vs preset stat scaling effectiveness
...
| APPLY-01 | Phase 5 | Complete |
| APPLY-02 | Phase 5 | Complete |
| APPLY-03 | Phase 5 | Complete |
| APPLY-04 | Phase 5 | Complete |
| APPLY-05 | Phase 5 | Complete |
