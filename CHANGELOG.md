...
### Added
- **Publish stamp** — the runtime export and the R2 publish write a `bundle` block into the config (`ambonmud.bundle`: id, source repo commit and dirty flag, world and config sha256, Arcanum version, export time) and `bundle: <id>` into every zone file, one id over the unstamped zones and config; the server logs it at boot, exports `ambon_bundle_info` and refuses a config from one publish with zones from another. Publish the config and the zones together.
