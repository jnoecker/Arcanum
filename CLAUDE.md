...
- **State** — Zustand stores in `src/stores/`, each independent. No middleware chaining and no cross-store subscriptions; stores read each other via `useOtherStore.getState()` when they need to.
  ...
  - `questAuthoringStore` — cross-zone quest authoring panel state
  - `sidebarStore` — sidebar drill target and collapse state (the six-island left rail)
  - `voiceStore` — ElevenLabs voice map, per-mob delivery settings, dialogue clip generation status
...
- **YAML I/O** — `yaml` package in CST mode for format-preserving round-trip. Loaders in `src/lib/loader.ts`, savers in `src/lib/saveZone.ts` and `src/lib/saveConfig.ts`. Zone key ordering is fixed by the object-assembly order in `sanitizeZone.ts` (`serializeZone` runs the world through `sanitizeZone` before `stringify`): `zone, startRoom, rooms, lifespan, terrain, graphical, pvpEnabled, levelBand, difficultyHint, faction, image, audio, video, mobs, items, shops, trainers, quests, gatheringNodes, recipes, puzzles, dungeon`. Only serialize non-zero values in StatMap fields. `trainers` is a synthesized server-facing map built from mobs with `role: trainer` — Arcanum-only mob trainer fields are stripped on save.
- **Project formats** — Two shapes, detected by the `validate_project` Rust command and dispatched on `project.format` (`"legacy" | "standalone"`) by every loader/saver. `projectPaths.ts` centralizes path resolution. **Legacy** = a Gradle-based AmbonMUD checkout with a monolithic `application.yaml`. **Standalone** = a flat directory with split config under `config/` plus `zones/<name>/zone.yaml`. The split is two-tier (`projectPaths.ts`): 11 **entity config files** loaded by exact name with file-scoped keys (`classes`, `races`, `abilities`, `status-effects`, `stats`, `equipment`, `combat`, `crafting`, `progression`, `assets`, `pets`) and 6 **world-pool files** merged by top-level key with canonical homes in `WORLD_SECTION_HOMES` (`server`, `commands`, `social`, `economy`, `quests`, `world`). Any other `.yaml` dropped in `config/` merges into the world pool too (drop-in sharing); the canonical file wins duplicates with a warning. A legacy fat `world.yaml` loads unchanged and is split on first save. MUD export (`exportMud.ts`) rebuilds a monolithic config from a standalone project.
...
- **Panel registry** — `src/lib/panelRegistry.ts` defines ~55 unique panels across 7 sidebar groups (`studio`, `character`, `world`, `systems`, `lore`, `operations`, `command`). Each panel has a `host` type (`studio` / `config` / `lore` / `command`) that `MainArea.tsx` uses to route to the right container, plus an `island` (`arcanum`, `forge`, `loom`, `orrery`, `livingWorld`, `spire`, or `settings`) that places it under one of the six World Map realms (settings panels are reached via the Spire / command palette rather than a map island). A panel may appear in more than one sidebar group but is deduped by `id`.
...
- **Lore system** — Article-based worldbuilding with 12 active built-in templates (world setting, character, location, organization, ancestry, bestiary, item, event, language, class, occupation, freeform) plus a Cinematic Story template, six retained legacy aliases (`species`, `profession`, `ability`, etc. — kept so older articles round-trip after the Species→Ancestry/Bestiary, Profession→Class/Occupation, Ability→Talent/Creature-Power splits), and user-defined custom templates (via `TemplateEditorPanel`). TipTap rich text, @mentions, interactive maps (Leaflet CRS.Simple), timeline with eras, relationship graph, multi-image galleries. AI tools: timeline inference, relationship inference (deterministic + field-based), gap analysis, consistency auditing, @mention suggestions, rewrite-with-instructions, and a "Ask your world" lore chat assistant. All AI writing retrieves context from the lore RAG index (see below). Bulk ops: multi-select, retag, reparent, delete, draft toggle, template change. Full-text search, Obsidian import, Lore Bible export to Markdown and PDF, command palette (Ctrl+K). Types in `src/types/lore.ts`, store in `src/stores/loreStore.ts`, persistence in `src/lib/lorePersistence.ts`.
- **Lore RAG** — Every article, timeline event, map pin, region, relationship, and entity description is chunked (`src/lib/rag/chunker.ts`) and embedded with Voyage AI `voyage-3-lite` (512-dim), stored locally per project. The index auto-reindexes (debounced) on lore edits after the first build; clearing it is a real off-switch. All AI writing paths (article rewrite/generation, consistency audit, gap analysis, @mention suggestions, timeline/relationship inference, zone/quest generation) retrieve from the index instead of stuffing the prompt. Frontend: `src/lib/rag/` (`autoIndex.ts`, `chunker.ts`, `index.ts`, `loreContext.ts`, `promptAssembly.ts`). Backend: `embeddings.rs` (Voyage client) + `rag.rs`. Embedding can run via direct Voyage key or the hub AI proxy. Status surfaced in the Lore Index settings panel.
- **Dialogue voice-over** — ElevenLabs TTS for NPC dialogue, with per-mob voice + delivery settings. Synthesized clips publish to R2 at a content-addressed path so edited lines never serve stale audio. Store: `voiceStore`. Backend: `elevenlabs.rs` (synthesis) + `voices.rs` (voice list) + the voice publish path in `r2.rs`. Cross-repo contract with the AmbonMUD server.
...
- `image_profiles.rs` — per-asset-type runtime image profiles (dimension caps + byte budgets per the AmbonMUD art contract: scenes ≤750 KiB preferred / 1.25 MiB ceiling, widgets ≤150 KiB) + downscale/WebP helpers, shared by the ingest paths and the R2 optimizer. `optimize_image_bytes` converts over-budget PNG/JPEG to lossy WebP, stepping quality down then dimensions until the budget fits; `player_sprite` is exempt (the `player_sprites/<key>.png` deploy path is a PNG contract). Always compiled (not behind the `ai` feature) because `assets.rs` and `r2.rs` use it in the Community Edition build
- `asset_migration.rs` — one-shot library migration ("Optimize Library" in the Asset Gallery): downscales stored images that exceed their runtime profile, converts over-budget PNG/JPEG to WebP, takes a project snapshot, updates the manifest, and rewrites every YAML/JSON reference (zones, config, lore, story files) via exact-token replacement (content-hash filenames are globally unique, so plain substitution is safe and format-preserving — extension changes ride along because the full filename is the token). Old files are deleted only after the manifest and all references point at the new names. Built for large libraries: re-encodes on a bounded parallel pool, checkpoints the manifest plus an old→new journal (`.arcanum/asset-migration-journal.json`) every 50 images so an interrupted run resumes on the next run, and supports mid-run cancellation (`cancel_asset_migration`) that finalizes the completed subset
...
- `anthropic.rs` — Anthropic Claude client (text + vision). Default model when no override is set: `claude-sonnet-4-6`.
- `openrouter.rs` — OpenRouter LLM client
- `llm.rs` — LLM dispatcher for prompt enhancement and vision analysis. Dispatches across Anthropic, OpenRouter, OpenAI, and DeepInfra providers; short-circuits to `hub_ai` when `settings.use_hub_ai` is on.
- `embeddings.rs` — Voyage AI embedding client for the lore RAG index (`voyage-3-lite`)
- `rag.rs` — lore RAG index storage/retrieval backend
- `elevenlabs.rs` — ElevenLabs text-to-speech client for dialogue voice-over
- `voices.rs` — ElevenLabs voice catalog + per-project voice map
- `snapshots.rs` — periodic project snapshots + zip archive export (offline backup)
...
- `r2.rs` — also owns the dialogue voice-over publish path (`deploy_voices_to_r2`): uploads synthesized clips to `voices/<zone>/<templateKey>/<nodeId>.<text_sha8>.mp3`, where the text hash in the key means edited lines publish to fresh paths and never serve stale audio.
...
- Pages (`showcase/src/pages/`): Home, Codex (`ArticlesPage`), Article detail (`ArticlePage`), Maps, Timeline, Connections (`GraphPage`), Stories (`StoriesPage` + `StoryPlayerPage`), `HubIndexPage`, plus hub account flows (`AccountPage`, `SignupPage`, `PrivacyPage`) and 404.
...
- **Self-registration** (public, `api.<root>`):
  - Tiers (defaults in `hub-worker/src/db.ts`): `demo` (self-signup, 10 images/20 prompts, no publish), `full` (email-verified, 500 images/1000 prompts, publish), `playtester` (admin-issued, 2000 images/20000 prompts), `publish` (admin-issued BYOK, publish-only, 0/0).
...
- **AI proxy** (Bearer auth, per-user lifetime quotas. Defaults by tier in `hub-worker/src/db.ts`: demo 10 images / 20 prompts, full 500 images / 1000 prompts, playtester 2000 / 20000, publish 0 / 0. Stored on `users`, reset on API key rotation):
  - `POST /ai/image/generate` → Runware (`runware:400@1`, `runware:400@2` FLUX.2, `openai:gpt-image@2` GPT Image 2)
  - `POST /ai/llm/complete` → Claude Sonnet 4.6 via direct Anthropic API (`LLM_MODEL = "claude-sonnet-4-6"` in `handlers/ai.ts` — chosen over a cheaper model for better zone generation and structured JSON; the older DeepSeek-via-OpenRouter and Haiku paths are gone)
