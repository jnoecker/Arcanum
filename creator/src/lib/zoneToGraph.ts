...
function buildGraphFingerprint(world: WorldFile): string {
  ...
  for (const [id, room] of Object.entries(world.rooms)) {
    const flags = `${room.bank ? 1 : 0}${room.tavern ? 1 : 0}${room.inn ? 1 : 0}${room.dungeon ? 1 : 0}${room.auction ? 1 : 0}${room.stylist ? 1 : 0}${room.housingBroker ? 1 : 0}${room.akathavaeShrine ? 1 : 0}${room.flightMaster ? 1 : 0}${room.boatDock ? 1 : 0}`;
    let exits = "";
    if (room.exits) {
